...
- **Database**: EF Core InMemory. No external database required. Demo data seeded on first run.
...
**Configuration Structure** - `EntraIdApiOptions` inherits from `EntraIdBaseOptions` (`McpBaseline.Shared`):

- **EntraIdBaseOptions** (abstract, in `McpBaseline.Shared/Configuration/`): Common properties
...
- **EntraIdApiOptions** (concrete, in `McpBaseline.MockApi/Configuration/`): API-specific properties
...
├── Data/
│   ├── MockApiDbContext.cs        # EF Core DbContext (InMemory): Tasks, Projects, Users, Balances
...
**Note**: `EntraIdApiOptions` is in `Configuration/` (this project), inheriting from `EntraIdBaseOptions` in `McpBaseline.Shared/Configuration/`. No custom validators are needed; DataAnnotations provide all validation.

### Design Decisions

- **EF Core InMemory**: lightweight in-process store; no external database required. Data is ephemeral and recreated on each application restart. EF Core InMemory operations are not captured by `AddEntityFrameworkCoreInstrumentation()` since there is no real database driver.
- **Fluent API over Data Annotations**: all schema constraints (column lengths, defaults, indexes, keys) are centralized in `EntityConfigurations/` via `IEntityTypeConfiguration<T>` and auto-applied via `ApplyConfigurationsFromAssembly()`. Entities are clean POCOs with no persistence concerns.
- **`DatabaseSeedingService`**: `IHostedService` that runs `EnsureCreatedAsync()` then `DbSeeder.SeedData()` sequentially before the app starts accepting requests. This guarantees tables exist before any controller handles a request.
...
Logging is fully centralized in `McpBaseline.ServiceDefaults` via the `AddSerilogDefaults()` extension method. MockApi's `Program.cs` simply calls:
...
- **File sink**: `RenderedCompactJsonFormatter` (structured JSON), daily rolling, 5-file retention (`logs/mcpbaseline-mockapi-*.log`)
...
Traces and metrics use the standard OpenTelemetry .NET SDK configured in `McpBaseline.ServiceDefaults`:
...
**Custom Metrics** (meter: `McpBaseline.MockApi`):
...
Locally these flow to the Aspire Dashboard. In production, the [DogStatsDMetricBridge](../McpBaseline.ServiceDefaults/README.md#dogstatsd-metric-bridge) forwards them to Datadog via named pipes as Distribution metrics, so counters report integer totals via the `count` aggregation.

| Variable                        | Purpose                       | Default                                 |
| ------------------------------- | ----------------------------- | --------------------------------------- |
| `OTEL_EXPORTER_OTLP_ENDPOINT`   | Collector endpoint            | _(disabled when empty)_                 |
| `OTEL_EXPORTER_OTLP_PROTOCOL`   | Wire protocol                 | `grpc` (local) / `http/protobuf` (prod) |
| `OTEL_SERVICE_NAME`             | Service identity              | `mock-api`                              |
| `OTEL_SEMCONV_STABILITY_OPT_IN` | OTel DB attribute conventions | _(see note below)_                      |

> **`OTEL_SEMCONV_STABILITY_OPT_IN` scope**: this variable only affects the **OTel SDK export path** (observable locally via the Aspire Dashboard). In production, `OTEL_EXPORTER_OTLP_ENDPOINT` is not set, so the OTel SDK is inactive. Datadog in production uses the CLR profiler with its own attribute schema regardless of this setting. See `McpBaseline.ServiceDefaults/README.md` for the full explanation.

See `McpBaseline.ServiceDefaults/README.md` for shared telemetry configuration.
