...
- [X] T011 [US2] Add client ID outputs to `infra/terraform/outputs.tf` — `backend_api_client_id` (`azuread_application.backend_api.application_id`), `mcp_server_client_id` (`azuread_application.mcp_server.application_id`), `foundry_agent_client_id` (`azuread_application.agent.application_id`)
...
- [X] T016 [US4] Add `backend "azurerm" {}` partial configuration block inside the `terraform {}` block in `infra/terraform/main.tf`
...
- [X] T017 Run `terraform fmt` (auto-fix mode) across `infra/terraform/`
