# McpBaseline.Presentation — Presentation Layer

## Overview

Outermost layer of the Clean Architecture. Acts as the composition root and MCP transport host. Contains MCP tools, prompts, middleware, telemetry filter, and all ASP.NET Core / MCP SDK wiring. No business logic lives here: tools delegate to use cases in `McpBaseline.Application`.

- **Target Framework**: .NET 10 (Web SDK)
- **NuGet packages**: `ModelContextProtocol.AspNetCore` v1.2.0
- **Project references**: `McpBaseline.Application`, `McpBaseline.Infrastructure`, `McpBaseline.ServiceDefaults`, `McpBaseline.Shared`
...
Four sealed tool classes, one per domain. Each class uses primary constructor to inject use cases (from `McpBaseline.Application/UseCases/`). Every method delegates to exactly one use case via `ExecuteAsync()` and returns `result.ToJson()`.
...
**Log SourceContext**: `McpBaseline.Presentation.Telemetry.McpTelemetryFilter` (the class emitting the log, not the tool class). The tool name is a structured property `{ToolName}` filterable in Serilog/Seq/Aspire Dashboard.
...
Centralized in `McpBaseline.ServiceDefaults` via `AddSerilogDefaults()`. The MCP Server calls `builder.Host.AddSerilogDefaults()` and `app.UseSerilogRequestLogging()`. No Serilog section in `appsettings.json`.
...
**Custom Metrics** (meter: `McpBaseline.Presentation`):
...
### Dual Instrumentation (MCP Server Only)

The MCP Server requires `DD_TRACE_OTEL_ENABLED=true` so the Datadog CLR profiler accepts OTel SDK spans. This is needed because Copilot Studio sends [POST-only requests](https://learn.microsoft.com/microsoft-copilot-studio/agent-extend-action-mcp) to `/mcp` without SSE, and the CLR profiler does not produce traces for this pattern. `DD_TRACE_ASPNETCORE_ENABLED=false` prevents duplicate spans on requests that the CLR profiler does instrument (VS Code SSE sessions).

Standalone CLR profiler traces for infrastructure calls are filtered via `DD_APM_IGNORE_RESOURCES`:

```
DD_APM_IGNORE_RESOURCES=^GET /(health|alive)?$|^TLS client handshake|^GET login\.microsoftonline\.com|datadoghq\.com
```

See `McpBaseline.ServiceDefaults/README.md` for the full architecture.

### Platform Routing

| Scenario                       | Protocol      | Target                    |
| ------------------------------ | ------------- | ------------------------- |
| Local (Aspire, no Datadog key) | gRPC          | Aspire Dashboard          |
| Local (Aspire + Datadog key)   | gRPC          | Datadog Agent container   |
| Production (Azure App Service) | http/protobuf | Datadog Agent App Service |
...
├── McpBaseline.Presentation.csproj
...
McpBaseline.Infrastructure
    ↑
McpBaseline.Presentation  (this project)
```

For layer details: [Domain README](../McpBaseline.Domain/README.md), [Application README](../McpBaseline.Application/README.md), [Infrastructure README](../McpBaseline.Infrastructure/README.md).
