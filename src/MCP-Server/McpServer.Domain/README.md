# McpServer.Domain — Business Constants and Rules
...
MCP Server App Role constants used in `[Authorize(Roles = ...)]` attributes across all Tool and Prompt classes. Values use the `mcp:` prefix to distinguish them from MockApi's own role set (see `McpServer.BackendApi/Constants/Permissions.cs`).
