# McpBaseline.Application — Use Cases, Contracts, Models, and Configuration

## Overview

Application layer in the Clean Architecture. Contains use cases (business logic and validation), service contracts (interfaces), the standardized tool result envelope, JSON serialization options, and downstream API configuration. Concrete infrastructure implementations live in `McpBaseline.Infrastructure`.

- **Target Framework**: .NET 10
- **NuGet packages**: `Microsoft.Extensions.DependencyInjection.Abstractions` v10.0.5
- **Project references**: `McpBaseline.Domain`
...
Use cases use `McpBaseline.Domain.Rules.TaskRules` for validation (e.g., `IsValidPriority()`, `IsValidStatus()`). They do NOT reference Infrastructure or telemetry; the `McpTelemetryFilter` captures tool-level failures.
...
Implementations: `EntraIdTokenExchangeService` (OBO via MSAL) in `McpBaseline.Infrastructure/Identity/`.
...
McpBaseline.Domain
    ↑
McpBaseline.Application  (this project)
    ↑
...
For Domain layer details, see `McpBaseline.Domain/README.md`.
